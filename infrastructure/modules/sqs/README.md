...
- Both queue names are env-suffixed by the caller (`LicenseQueue-dev`, `LicenseDeadLetterQueue-dev`), so dev and prod can coexist in one region. Renaming a queue is a **replace**, not an update — changing the suffix destroys and recreates both queues, and any in-flight messages are lost.